...
__all__ = ['LocalAlphaGemma2ForCausalLM']


class LocalAlphaGemma2DecoderLayer(Gemma2DecoderLayer):
    """
    Gemma-2 decoder layer with LocalAlphaSteer soft-gated local projectors.
    ...
    def forward(
        ...
        **kwargs,
        ...
        if hidden_states.shape[1] > 1 and self._should_apply_steering:
            ...
            ).to(hidden_states.dtype) * self.strength
...
class LocalAlphaGemma2Model(Gemma2Model):
    def __init__(self, config: Gemma2Config):
        super().__init__(config)
        self.layers = nn.ModuleList(
            [LocalAlphaGemma2DecoderLayer(config=config, layer_idx=i)
    ...
    def set_steering_parameters(
        ...
        for layer_idx, layer in enumerate(self.layers):
            layer_centroids = centroids[layer_idx] if centroids is not None else None
            layer_matrices = (
                local_steering_matrices[layer_idx]
                if local_steering_matrices is not None
                else None
            )
            layer_strength = strength[layer_idx] if strength is not None else 0.0
    ...
    def _print_steering_parameters(self):
        logger.info("LocalAlphaSteer Parameters (Gemma2):")
...
class LocalAlphaGemma2ForCausalLM(Gemma2ForCausalLM):
    def __init__(self, config: Gemma2Config):
        super().__init__(config)
        self.model = LocalAlphaGemma2Model(config=config)
