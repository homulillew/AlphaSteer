...
__all__ = ['LocalAlphaQwen2ForCausalLM']


class LocalAlphaQwen2DecoderLayer(Qwen2DecoderLayer):
    """
    Qwen2 decoder layer with LocalAlphaSteer soft-gated local projectors.
...
class LocalAlphaQwen2Model(Qwen2Model):
    def __init__(self, config: Qwen2Config):
        super().__init__(config)
        self.layers = nn.ModuleList(
            [LocalAlphaQwen2DecoderLayer(config=config, layer_idx=i)
    ...
    def _print_steering_parameters(self):
        logger.info("LocalAlphaSteer Parameters (Qwen2):")
...
class LocalAlphaQwen2ForCausalLM(Qwen2ForCausalLM):
    def __init__(self, config: Qwen2Config):
        super().__init__(config)
        self.model = LocalAlphaQwen2Model(config=config)
